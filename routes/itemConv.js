...
router.get('/createFBDI', async function(req, res, next) {
  ...
  /* UDA */
  zip = new admZip()
  const dffCsv = await utils.getCsvForm(preRequisiteData[3].data) 
  const dffContextCsv = await utils.getCsvForm(preRequisiteData[4].data) 
  const dffContextCategoryCsv = await utils.getCsvForm(preRequisiteData[5].data) 
  const dffContextSegmentCsv = await utils.getCsvForm(preRequisiteData[6].data) 
  const dffContextUsageCsv = await utils.getCsvForm(preRequisiteData[7].data) 
  const dffPageCsv = await utils.getCsvForm(preRequisiteData[8].data) 
  const dffPageTaskFlowCsv = await utils.getCsvForm(preRequisiteData[9].data) 
  const dffSegmentCsv = await utils.getCsvForm(preRequisiteData[10].data) 
  const dffUsageCsv = await utils.getCsvForm(preRequisiteData[11].data) 
